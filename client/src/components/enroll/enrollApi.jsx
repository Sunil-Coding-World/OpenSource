export async function AddtoEnroll(item) {
  try {
    const response = await fetch('https://opensource-server.vercel.app/enroll', {
      method: "POST",
...
export async function fetchItemsByUserId(userId) {
  try {
    const response = await fetch(`https://opensource-server.vercel.app/enroll/${userId}`);
    const data = await response.json();
    return { data };
  } catch (error) {
...
export async function deleteItemFromEnroll(userId,courseId) {
  try {
    const response = await fetch(`https://opensource-server.vercel.app/enroll/${userId}/${courseId}`,
      {
      method: 'DELETE',
      headers: { 'content-type': 'application/json' },
    });
